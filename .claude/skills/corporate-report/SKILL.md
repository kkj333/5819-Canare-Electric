---
name: corporate-report
description: 上場企業の分析レポートとチャートを生成する。使い方 /corporate-report [証券コード] [企業名]
---

# 企業分析レポート生成

証券コード `$0`、企業名 `$1` の企業分析レポートを作成する。

**実例**: `5819_canare/report.md` と `5819_canare/charts.html` を参照すること。構成・トーン・ハイライトの使い方はこの実例に合わせる。

---

## 1. フォルダ作成

`{証券コード}_{企業名英語小文字}/` を作成（例: `5819_canare/`）。その中に:

| ファイル | 内容 |
|---|---|
| `README.md` | 企業トップページ。`[← 企業一覧](../)` リンク付き |
| `report.md` | 分析レポート本体 |
| `charts.html` | Chart.js グラフページ。Jekyll front matter `layout: default` 付き |

作成後、ルート `README.md` の企業一覧テーブルに行を追加する。

---

## 2. データ収集チェックリスト

レポートに必要なデータ。有価証券報告書・決算短信・IR資料から収集する。

- [ ] 会社概要（社名、コード、本社、代表者、設立、決算月、従業員数、時価総額）
- [ ] 沿革（創業〜現在）
- [ ] 製品構成（売上・構成比・前期比）
- [ ] セグメント構成（地域別 or 事業別）
- [ ] 連結経営指標 5期以上（売上、経常利益、純利益、BPS、EPS、ROE、自己資本比率）
- [ ] 連結PL詳細 2期（売上原価、販管費、営業利益、営業外、特別損益、法人税）
- [ ] 連結BS詳細 2期（流動資産内訳、固定資産内訳、負債内訳）
- [ ] 連結CF 長期推移（営業CF、投資CF、財務CF、設備投資、現金残高）
- [ ] 四半期推移（売上、営業利益、営業利益率、原価率、販管費率）
- [ ] 大株主リスト、配当推移、資本政策方針、自社株買い実績
- [ ] ガバナンス（取締役構成、監査法人、KAM、関連当事者取引）
- [ ] 実効税率（ROIC計算に必要。2期分以上）
- [ ] 市場規模・競合情報
- [ ] 有報記載のリスク・特記事項（棚卸評価損、税務調査、後発事象等）
- [ ] 業績予想（会社予想）、過去の予想精度

---

## 3. レポート構成（report.md）

先頭にナビリンクとCSSを置く:

```markdown
[← {企業名}トップ](./) ｜ [財務データグラフ](charts.html) ｜ [企業一覧](../)

<style>
.hl-red { background: #fdecea; color: #c0392b; font-weight: 600; padding: 0.1em 0.4em; border-radius: 3px; }
.hl-green { background: #eafaf1; color: #1e8449; font-weight: 600; padding: 0.1em 0.4em; border-radius: 3px; }
.hl-amber { background: #fef9e7; color: #b7950b; font-weight: 600; padding: 0.1em 0.4em; border-radius: 3px; }
</style>

<div class="hl-legend">
<span><span class="dot" style="background:#e74c3c"></span> 割安シグナル・要注目</span>
<span><span class="dot" style="background:#f39c12"></span> 改善余地あり</span>
<span><span class="dot" style="background:#27ae60"></span> ポジティブ</span>
</div>
```

### セクション構成

```
# {企業名}（{証券コード}）企業分析レポート

## 会社概要
## 沿革
## 事業内容
  ### 製品構成
  ### セグメント構成
## 市場規模・競合
## 業績推移
  ### 連結経営指標等の推移（5期以上）
  ### ROE vs 事業ROIC
  ### 連結損益計算書
  ### 四半期推移（QonQ）
  ### セグメント別業績
## 要約財務諸表
  ### 連結BS推移・詳細
  ### 連結PL詳細
  ### 連結CF計算書（10年推移）
  ### 運転資本効率
## 株主動向・資本政策
  ### 大株主リスト
  ### 配当推移
  ### 資本政策の基本方針
  ### コーポレートガバナンス
## 業績予想
  ### 会社予想の保守性（過去の予想 vs 実績）
## 企業価値
  ### 資産バリューチェック（清算価値）
  ### 収益バリューチェック（DCF法）
    - 弱気: NC + FCF/R（成長なし）
    - ミドル: 5年10%成長（利益CAGRに近い現実的シナリオ）
    - 強気: 5年20%成長（楽観前提を必ず明記）
    - シナリオ比較表
## 投資判断
  ### 五段階評価（A〜E）
  ### 総括（重要リスクもここに言及する）
  ### 株価が評価されにくい構造的背景
  ### ブルケース
## 事業等のリスク
## 株価指標
```

---

## 4. 分析ルール

### ROIC（必須）

```
事業ROIC = NOPAT ÷ 事業投下資本
NOPAT = 営業利益 ×（1 - 実効税率）
事業投下資本 = 純資産 - ネットキャッシュ
```

- **絶対に純利益ベースで計算しない**。営業外損益・税効果が混在して実態がぶれる
- 営業利益が直接取れない年度は経常利益からの推計とし、`*` マークで明示する
- 実効税率はレポート内に明記する（年度ごとに異なる場合がある）

### DCF（3シナリオ必須）

| シナリオ | 内容 | 注意 |
|---|---|---|
| 弱気 | NC + FCF/R（成長ゼロ） | 保守的な下限値 |
| ミドル | 5年10%成長 | 利益CAGRに近い想定。**最も重視** |
| 強気 | 5年20%成長 | 実績CAGRとの乖離を必ず明記 |

- 3シナリオの比較表を必ず付ける
- 割引率は10%を基準とし、前提条件を冒頭にまとめる

### ベアケース vs ブルケース

- **ベアケース**: 事実ベースで構造的課題を整理する。攻撃的・断定的にならない。応援目線で「なぜ評価されにくいか」を冷静に分析する
  - 株価推移の事実（高値更新の有無、期間）
  - 株主総利回り vs TOPIX
  - 株主構成と還元インセンティブの構造
  - カタリスト候補があれば、期待と不確実性の両面を書く
- **ブルケース**: 希望ではなく実績データで裏付ける。「いつか変わる」だけでは不十分

### カラーハイライトの基準

テーブル内の数値に `<span class="hl-red">` 等を使う:

| 色 | 使いどころ | 例 |
|---|---|---|
| 赤 `hl-red` | 割安シグナル、要注目リスク | PBR<1、EV<0、急落した利益率、評価損急増 |
| 黄 `hl-amber` | 改善余地、注意 | ROE低水準、予想減益、季節性パターン |
| 緑 `hl-green` | ポジティブ | 成長セグメント、高い評価（A/B）、財務健全性 |

- 付けすぎない。1テーブルに2〜3箇所が目安
- 重要リスクは総括セクションにも言及する（リスクセクションだけに埋もれさせない）
- 推計値には必ず `*` を付けて注記する

---

## 5. チャート（charts.html）

Chart.js v4 + chartjs-plugin-annotation。Jekyll front matter 必須:

```yaml
---
layout: default
title: {企業名} - 財務データグラフ
---
```

ナビリンク:
```html
<p><a href="./">← {企業名}トップ</a> ｜ <a href="report.html">企業分析レポート</a> ｜ <a href="../">企業一覧</a></p>
```

### 推奨グラフ（企業に応じて取捨選択）

1. **売上高・利益の推移** — 棒（売上）+ 折れ線（経常利益・純利益）の複合
2. **セグメント別 売上高・利益** — 地域別 or 事業別
3. **四半期 営業利益率** — 季節性があればポイント色で強調、平均ライン注釈
4. **BPS vs 株価** — PBR圧縮の可視化
5. **ネットキャッシュ vs 時価総額** — NC > 時価総額の年度を赤色に
6. **営業CF・実質FCF** — 実質FCF = 営業CF - 設備投資
7. **ROE vs 事業ROIC** — 8%参照ライン、乖離を示すラベル注釈
8. **海外売上比率**（該当する場合）— 50%等の節目ライン
9. **配当の推移** — 配当性向を折れ線で重ねる

### グラフの装飾ルール

- チャートコンテナに左ボーダー色: `hl-red` / `hl-amber` / `hl-green`
- annotation プラグインで参考ライン（8%、50%等）やゾーン（未達ゾーン）
- 注目データポイントのポイント色を変更（低い値を赤、高い値を緑）
- 注釈テキスト `.chart-note` にも色付きクラスを付与
- ページ上部にカラー凡例を配置

---

## 6. ファイル名・リンク規約

- ディレクトリ名・ファイル名は**英語のみ**（日本語はURLエンコードで問題になる）
- ディレクトリ命名: `{証券コード}_{企業名英語小文字}`（例: `5819_canare`, `7203_toyota`）
- report.md 内のナビ: `[← {企業名}トップ](./) ｜ [財務データグラフ](charts.html) ｜ [企業一覧](../)`
- charts.html 内のナビ: `<a href="./">← {企業名}トップ</a> ｜ <a href="report.html">レポート</a> ｜ <a href="../">企業一覧</a>`
